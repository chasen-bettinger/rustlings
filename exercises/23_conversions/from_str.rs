...
impl FromStr for Person {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        let (Some(name), Some(age), None) = (split_str.next(), split_str.next(), split_str.next())
        else {
        ...
        let Ok(parsed_age) = age.parse::<u8>() else {
            return Err(ParsePersonError::ParseInt(age.parse::<u8>().err().unwrap()));
        };
